...
fit.int = lm(alcohol ~ . ^ 2, data = data.train)           # 2nd order interaction
#fit.int = lm(alcohol ~ sugar*density + sugar*pH + sugar*sulphates + 
#                density*pH + density*sulphates + pH*sulphates, data = data.train)
...
library(caret)
RMSE
RMSE(pred.sugar, Y.valid)^2
RMSE(pred.density, Y.valid)^2
RMSE(pred.pH, Y.valid)^2
RMSE(pred.sulphates, Y.valid)^2
RMSE(pred.all, Y.valid)^2
RMSE(pred.int, Y.valid)^2
##############
